 ...
 * Proves HTTP status codes, security filter, validation handler, anti-enumeration,
...
class ExamControllerIntegrationTests {
    ...
    @BeforeEach
    void setUp() {
        teacherUserId = insert("INSERT INTO users (username, email, password_hash, display_name) VALUES ('ct','ct@t','h','CT')");
        ...
        teacherProfileId = insert("INSERT INTO teacher_profiles (user_id, school_id, teacher_code) VALUES (" + teacherUserId + "," + schoolId + ",'CTC')");
    }
    ...
    @Test
    void postExamBlankCodeReturns400ExamValidationError() throws Exception {
        mockMvc.perform(post("/api/exams")
                .with(jwt().jwt(j -> j.subject(teacherUserId.toString())))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void getExamDetailForeignOwnerReturns404ExamNotFound() throws Exception {
        ...
        long otherUser = insert("INSERT INTO users (username, email, password_hash, display_name) VALUES ('o2','o2@t','h','O2')");
        jdbc.update("INSERT INTO user_roles (user_id, role_id) SELECT ?, id FROM roles WHERE code='TEACHER'", otherUser);
        insert("INSERT INTO teacher_profiles (user_id, school_id, teacher_code) VALUES (" + otherUser + "," + schoolId + ",'TC2')");
