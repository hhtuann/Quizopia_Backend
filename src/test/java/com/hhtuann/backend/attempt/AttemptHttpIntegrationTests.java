...
/**
 * MockMvc HTTP integration tests for the two A3.2-1 endpoints. Uses Spring Security's
 * jwt() post-processor (no real login). Each error test asserts exact HTTP status +
 * exact code in JSON body. No answerKey/isCorrect/explanation/score in responses.
 */
@SpringBootTest
...
class AttemptHttpIntegrationTests {
    ...
    void setUp() {
        ...
        sessionId = insert("INSERT INTO exam_sessions (school_id, exam_version_id, owner_teacher_id, code, title, status, starts_at, ends_at, max_attempts, created_by, opened_at) VALUES ("
                + school + "," + ver + "," + tp + ",'S','Sess','OPEN','" + baseTime.minusSeconds(3600) + "','" + baseTime.plusSeconds(7200) + "',1," + studentUserId + ",'" + baseTime.minusSeconds(3600) + "')");
    ...
    void startReturns409OutsideWindow() throws Exception {
        jdbc.update("UPDATE exam_sessions SET starts_at='" + baseTime.plusSeconds(3600) + "' WHERE id=" + sessionId);
        mockMvc.perform(post("/api/exam-sessions/" + sessionId + "/attempts").with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.jwt()
                        .jwt(j -> j.subject(jwt())))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void startReturns409MaxReached() throws Exception {
        ...
        jdbc.update("UPDATE attempts SET status='SUBMITTED', submitted_at=now(), submission_idempotency_key='MAXTEST' WHERE id=" + attemptId);
    ...
    void startReturns409AfterEndsAt() throws Exception {
        jdbc.update("UPDATE exam_sessions SET ends_at='" + baseTime.minusSeconds(1) + "' WHERE id=" + sessionId);
        mockMvc.perform(post("/api/exam-sessions/" + sessionId + "/attempts").with(
                org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.jwt()
                        .jwt(j -> j.subject(jwt())))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    private org.springframework.security.test.web.servlet.request.SecurityMockMvcRequestPostProcessors.JwtRequestPostProcessor jwtWithSubject(long userId) {
