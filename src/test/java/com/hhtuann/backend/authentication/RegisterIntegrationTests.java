...
class RegisterIntegrationTests extends AbstractAuthenticationIntegrationTests {
    ...
    void nullAccountTypeDefaultsToStudent() throws Exception {
        String json = """
                {"username":"%s","email":"%s","password":"Passw0rd!","displayName":"Default User","phone":"+8499"}
    ...
    void usernameContainingAtSignIsRejected() throws Exception {
        String json = studentRegisterJson("has@at", emailFor(unique("at")), "Passw0rd!", "At User", "+8499");
        int status = mockMvc.perform(post("/api/auth/register")
                        .contentType(org.springframework.http.MediaType.APPLICATION_JSON)
                        .content(json))
                .andReturn().getResponse().getStatus();
    ...
    void passwordShorterThanEightIsRejected() throws Exception {
        String json = studentRegisterJson(unique("shortpw"), emailFor(unique("shortpw")), "Short1!", "Short", "+8499");
        int status = mockMvc.perform(post("/api/auth/register")
                        .contentType(org.springframework.http.MediaType.APPLICATION_JSON)
                        .content(json))
                .andReturn().getResponse().getStatus();
    ...
    @Test
    void phoneIsStoredAsCiphertextAndPasswordIsHashed() throws Exception {
        String username = unique("cipher");
        registerStudent(username, "Passw0rd!");

        Object[] row = (Object[]) entityManager.createNativeQuery(
                        "SELECT password_hash, phone_encrypted FROM users WHERE username = :u")
    ...
    private MvcResultHolder registerStudentRaw(String username, String email) throws Exception {
        String json = studentRegisterJson(username, email, "Passw0rd!", username + " Name", "+84991234567");
    ...
    private MvcResultHolder registerTeacherRaw(String username, String invite) throws Exception {
        String json = teacherRegisterJson(username, emailFor(username), "Passw0rd!", username + " Name", "+84991234567", invite);
        return performRegister(json);
    }
