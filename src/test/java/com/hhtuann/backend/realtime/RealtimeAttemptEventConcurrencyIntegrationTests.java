...
class RealtimeAttemptEventConcurrencyIntegrationTests extends RealtimeStompTestBase {
    ...
    @Autowired private com.hhtuann.backend.realtime.support.RepositoryLockEntryProbe lockProbe;
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            try {
                ...
                Future<?> b = pool.submit(() -> {
                    lockProbe.armCurrentThread(com.hhtuann.backend.realtime.support.RepositoryLockEntryProbe.LockPoint.SESSION_FOR_UPDATE);
                    return attemptService.startAttempt(f.studentId, f.sessionId, new StartAttemptRequest(null));
                });
                assertThat(lockProbe.awaitEntered(com.hhtuann.backend.realtime.support.RepositoryLockEntryProbe.LockPoint.SESSION_FOR_UPDATE, 10)).as("worker B reached ExamSessionRepository.findByIdForUpdate").isTrue();
    ...
    void differentKeyConcurrentSubmitProducesOneEventPair() throws Exception {
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            try {
                ...
                long oks = (rb instanceof com.hhtuann.backend.attempt.dto.SubmitResponse ? 1 : 0);
    ...
    private static long countSubmitResponses(Future<Object> a, Future<Object> b) {
        long oks = 0;
        try { if (a.get(30, TimeUnit.SECONDS) instanceof com.hhtuann.backend.attempt.dto.SubmitResponse) oks++; } catch (Exception ignored) {}
        try { if (b.get(30, TimeUnit.SECONDS) instanceof com.hhtuann.backend.attempt.dto.SubmitResponse) oks++; } catch (Exception ignored) {}
        return oks;
    }
