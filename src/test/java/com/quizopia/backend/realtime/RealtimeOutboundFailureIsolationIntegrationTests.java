...
/**
 * Outbound failure-isolation closure (Day 7 B1R4-B §14-§17). Uses the
 * {@code @Primary} test decorators
 * ({@link FailingRealtimePublisher} +
 * {@link RecordingRealtimeActiveCountService}) to inject one-shot
 ...
 * <li>the independent second send is NOT suppressed by a failed first send
 * (failure isolation);</li>
 * <li>no retry / no duplicate;</li>
 * <li>the broadcaster's WARN log omits the injected sensitive marker (§17
 * sanitized logging).</li>
 * </ul>
 *
 * <p>
 * The service call with the failure injected IS the REST path (the controller
 * delegates to the
 * service; the broadcaster runs AFTER_COMMIT and cannot reach the response).
 * The §16 HTTP/MockMvc
 * surface adds only the security-filter + controller layer, which does not
 * interact with a post-commit
 * broker failure.
 */
...
class RealtimeOutboundFailureIsolationIntegrationTests extends RealtimeStompTestBase {
    ...
    @Autowired
    private FailingRealtimePublisher publisher;
    @Autowired
    private RecordingRealtimeActiveCountService activeCount;
    @Autowired
    ...
    void setUp() {
        ...
        ins("INSERT INTO student_profiles (user_id, school_id, student_code) VALUES (" + studentId + "," + school
                + ",'SC" + tag + "')");
        jdbc.update(
                "INSERT INTO exam_session_participants (school_id, exam_session_id, student_profile_id, added_by) VALUES ("
                        + school + "," + sessionId + ",(SELECT id FROM student_profiles WHERE user_id=" + studentId
                        + ")," + teacherId + ")");
        publisher.reset();
        activeCount.reset();
    }

    // §15.1 — attempt-event send fails: REST commits, ACTIVE_COUNT_CHANGED still
    // sent, no duplicate.
    @Test
    void attemptEventSendFailureRestCommitsAndCountStillSent() throws Exception {
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            Capture cap = subscribe(conn, 1); // the failed ATTEMPT_STARTED is dropped; only ACTIVE_COUNT_CHANGED
                                              // arrives
            awaitAccepted(acceptedDestination(topic(sessionId)), 5);

            long attemptId = attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null))
                    .attemptId();
            ...
            assertThat(cap.payloads)
                    .as("the failed ATTEMPT_STARTED is dropped; the independent ACTIVE_COUNT_CHANGED is still sent")
                    .hasSize(1);
            assertThat(parse(cap.payloads)[0].eventType()).isEqualTo(RealtimeEventType.ACTIVE_COUNT_CHANGED.name());
            // No retry: only one send attempt for ATTEMPT_STARTED (the failed one) —
            // recorded by the decorator.
            assertThat(publisher.sendAttempts().stream().filter(a -> a.type() == RealtimeEventType.ATTEMPT_STARTED)
                    .count())
                    .as("no retry of the failed attempt event").isEqualTo(1);
        }
    }

    // §15.2 — active-count send fails: attempt event still sent, REST commits, no
    // rollback.
    @Test
    void activeCountSendFailureAttemptEventStillSent() throws Exception {
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            Capture cap = subscribe(conn, 1); // the failed ACTIVE_COUNT_CHANGED is dropped; ATTEMPT_STARTED still
                                              // arrives
            awaitAccepted(acceptedDestination(topic(sessionId)), 5);
    ...
    // §15.4 — active-count query fails: REST commits, no fake count, both events
    // suppressed (documented behavior).
    @Test
    void activeCountQueryFailureRestCommitsAndSendsNoFakeCount() throws Exception {
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            ...
            // Documented behavior: the broadcaster's outer try-catch wraps count + both
            // sends, so a
            // count-query failure suppresses BOTH events (no fake/partial payload). REST is
            // source of truth.
    ...
    // §17 — sanitized logging: the injected sensitive marker must NOT appear in the
    // broadcaster's WARN log.
    @Test
    void sanitizedLoggingOmitsInjectedSensitiveMarker() throws Exception {
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            ...
            try (InMemoryLogAppender appender = InMemoryLogAppender.attach(
                ...
                attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null));
                // Harden: the appender MUST have captured at least one WARN (proves the failure
                // was logged).
                ...
                // The WARN must contain generic event type + session context (not the raw
                // exception).
    ...
    private long newScheduledSession() {
        ...
        long owner = jdbc.queryForObject("SELECT owner_teacher_id FROM exam_sessions WHERE id=" + sessionId,
                Long.class);
        return ins(
                "INSERT INTO exam_sessions (school_id, exam_version_id, owner_teacher_id, code, title, status, starts_at, ends_at, max_attempts, created_by) VALUES ("
                        + school + "," + ver + "," + owner + ",'FISCH','t','SCHEDULED','"
                        + Instant.now().minusSeconds(3600) + "','" + Instant.now().plusSeconds(7200) + "',2,"
                        + teacherId + ")");
    }

    private void assertNoOutboundForSession(long sid, long timeoutSeconds) {
        String d = topic(sid);
        assertNoOutbound(c -> c.command() == org.springframework.messaging.simp.stomp.StompCommand.MESSAGE
                && d.equals(c.destination()), timeoutSeconds);
    }
    ...
    private Capture subscribe(StompTestConnection conn, int expected, long sid) {
        ...
        conn.subscribe(topic(sid), new org.springframework.messaging.simp.stomp.StompFrameHandler() {
            @Override
            public java.lang.reflect.Type getPayloadType(org.springframework.messaging.simp.stomp.StompHeaders h) {
                return byte[].class;
            }

            @Override
            public void handleFrame(org.springframework.messaging.simp.stomp.StompHeaders h, Object payload) {
