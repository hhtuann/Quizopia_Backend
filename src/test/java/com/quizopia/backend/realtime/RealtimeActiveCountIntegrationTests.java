...
/**
 * Active-count closure (Day 7 B1R4-B §11-§13). The
 * {@link RecordingRealtimeActiveCountService}
 * decorator ({@code @Primary} via {@link RealtimeTestSupportConfig}) counts
 * {@code countActiveAttempts}
 * invocations and records the live transaction context, proving:
 * <ul>
 * <li>§11 correctness: start → activeCount N+1; submit → N-1; 2 active → submit
 * one → 1;</li>
 * <li>§12 fresh REQUIRES_NEW: each call runs in an active, read-only
 * transaction (a self-invocation
 * would have NO active transaction; the write tx already committed before
 * AFTER_COMMIT fires);</li>
 * <li>§13 query matrix: exactly one count invocation per committed attempt
 * transition; zero for
...
class RealtimeActiveCountIntegrationTests extends RealtimeStompTestBase {
    ...
    @Autowired
    private com.quizopia.backend.exam.application.ExamSessionService examSessionService;
    @Autowired
    ...
    void activeCountBeanIsAopProxyAndQueryIsSingleAggregateOnAttempts() throws Exception {
        // §7 — the @Primary decorator is a Spring AOP proxy (inherited @Transactional
        // honored).
        ...
        ch.qos.logback.classic.Logger sqlLogger = (ch.qos.logback.classic.Logger) org.slf4j.LoggerFactory
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            ...
            awaitAccepted(acceptedDestination(topic(sessionId)), 5);
            activeCount.reset();
            try (com.quizopia.backend.realtime.support.InMemoryLogAppender app = com.quizopia.backend.realtime.support.InMemoryLogAppender
                    .attach("org.hibernate.SQL")) {
                attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null));
    ...
    void writeAndCountUseDifferentTransactionResources() throws Exception {
        // §4 — direct resource-identity comparison: the write tx's ConnectionHolder !=
        // the count tx's.
        ...
        try (StompTestConnection conn = connect(teacherToken)) {
            ...
            try {
                pool.submit(() -> tx().executeWithoutResult(status -> {
                    attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null));
                    Object res = org.springframework.transaction.support.TransactionSynchronizationManager
                            .getResource(ds);
    ...
    void resumeRunsZeroActualActiveCountSql() throws Exception {
        // §5 — actual captured SQL proves 0 matching active-count statements for a
        // no-op resume.
        attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null));
        activeCount.reset();
        ch.qos.logback.classic.Logger sqlLogger = (ch.qos.logback.classic.Logger) org.slf4j.LoggerFactory
    ...
    /**
     * Counts SQL statements (one per Hibernate log event) matching the active-count
     * aggregate pattern.
     */
    ...
    // ============================
    // Every scenario proven by ACTUAL captured Hibernate SQL (org.hibernate.SQL at
    // DEBUG), not invocationCount.
    // The count query fires AFTER_COMMIT synchronously — no STOMP subscriber
    // needed; the SQL is logged
    // regardless of whether a client is listening. Bind values don't appear in
    // Hibernate SQL logs; the
    // status=IN_PROGRESS bind is proven by the repository method
    // countByExamSessionIdAndStatus(…, AttemptStatus.IN_PROGRESS).

    @Test
    void actualSqlSubmitCommitIsOne() {
        long attemptId = attemptService.startAttempt(studentId, sessionId, new StartAttemptRequest(null)).attemptId();
        activeCount.reset();
        long n = captureActiveCountSqlDuring(() -> submitService.submitAttempt(studentId, attemptId,
                new SubmitRequest("sql-submit-" + UUID.randomUUID())));
    ...
    void actualSqlCachedRetryIsZero() {
        ...
        long n = captureActiveCountSqlDuring(
                () -> submitService.submitAttempt(studentId, attemptId, new SubmitRequest("sql-cache"))); // cached
                                                                                                          // retry
    ...
    /**
     * Captures Hibernate SQL during {@code action} (logger at DEBUG, fresh appender
     * attached AFTER fixture
     * setup) and returns the number of statements matching the active-count
     * aggregate pattern.
     */
    ...
    private long newScheduledSession() {
        ...
        return ins(
                "INSERT INTO exam_sessions (school_id, exam_version_id, owner_teacher_id, code, title, status, starts_at, ends_at, max_attempts, created_by) VALUES ("
                        ...
                        + java.time.Instant.now().minusSeconds(3600) + "','" + java.time.Instant.now().plusSeconds(7200)
                        + "',2," + teacherId + ")");
    }
    ...
    /**
     * After the expected latch count is received, asserts no extra frame arrives in
     * a bounded window.
     */
    ...
    private long addStudentParticipant(String tag, long school) {
        ...
        jdbc.update(
                "INSERT INTO exam_session_participants (school_id, exam_session_id, student_profile_id, added_by) VALUES ("
                        + school + "," + sessionId + ",(SELECT id FROM student_profiles WHERE user_id=" + uid + "),"
                        + teacherId + ")");
        return uid;
    }
