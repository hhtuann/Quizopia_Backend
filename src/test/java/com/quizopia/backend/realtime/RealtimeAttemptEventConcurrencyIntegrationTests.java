...
/**
 * Realtime event-concurrency with LIVE STOMP capture (B1R4-B1 §1-§4).
 * Refactored from the DB-only
 * version to extend {@link RealtimeStompTestBase}: a teacher subscribes the
 * topic BEFORE the workers
 * fire, real STOMP frames are captured + parsed to
 * {@link RealtimeEventEnvelope}, and exact event
 * count/type/payload/no-duplicate is asserted alongside the DB result.
 *
 * <p>
 * <b>Fresh fixtures:</b> start scenarios begin from ZERO attempts (no
 * pre-existing IN_PROGRESS).
 * <b>True overlap:</b> the start scenarios use a held-transaction barrier —
 * worker A opens an outer
 * {@link TransactionTemplate}, calls {@code startAttempt} (acquires the
 * participant {@code FOR UPDATE}
 * lock), signals held, and waits; worker B calls {@code startAttempt} and
 * BLOCKS on that lock. The main
 * thread asserts B is not complete while A holds the lock (true overlap), then
 * releases A.
 *
 * <p>
 * <b>DELIVERY_ORDER_UNSPECIFIED</b> — the frozen contract (§9.6) fixes event
 * TYPES/nullability but
 * does not guarantee inter-event transport delivery order (the simple broker is
 * an async ExecutorChannel).
 * Assertions are by exact count + type + payload + distinct eventIds, NOT by
 * list index.
 */
...
class RealtimeAttemptEventConcurrencyIntegrationTests extends RealtimeStompTestBase {
    ...
    @Autowired
    private AttemptAutosaveService autosaveService;
    @Autowired
    private PlatformTransactionManager txm;
    @Autowired
    private ObjectMapper objectMapper;
    @Autowired
    private com.quizopia.backend.realtime.support.RepositoryLockEntryProbe lockProbe;
    ...
    private record ExistingAttempt(long sessionId, long teacherId, String teacherToken, long studentId, long attemptId,
            long aqId) {
    }

    // === §3.1 same-student concurrent start (FRESH, zero attempts) + true-overlap
    // barrier ===
    @Test
    void sameStudentConcurrentStartProducesOneEventPair() throws Exception {
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            try {
                // Worker A: outer tx → startAttempt (participant lock held) → signal → await
                // release.
                ...
                Future<?> b = pool.submit(() -> {
                    lockProbe.armCurrentThread(
                            com.quizopia.backend.realtime.support.RepositoryLockEntryProbe.LockPoint.SESSION_FOR_UPDATE);
                    return attemptService.startAttempt(f.studentId, f.sessionId, new StartAttemptRequest(null));
                });
                assertThat(lockProbe.awaitEntered(
                        com.quizopia.backend.realtime.support.RepositoryLockEntryProbe.LockPoint.SESSION_FOR_UPDATE,
                        10)).as("worker B reached ExamSessionRepository.findByIdForUpdate").isTrue();
    ...
    void sameKeyConcurrentSubmitProducesOneEventPair() throws Exception {
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            try {
                ...
                assertThat(lockProbe.awaitEntered(LockPoint.ATTEMPT_FOR_UPDATE, 10))
                        .as("worker B reached AttemptRepository.findByIdForUpdate").isTrue();
            ...
            try {
                Future<?> a = pool.submit(() -> {
                    tx().executeWithoutResult(status -> {
                        wrap(() -> submitService.submitAttempt(f.studentId, f.attemptId,
                                new SubmitRequest("ka-" + UUID.randomUUID())));
                        aHeld.countDown();
                        awaitLatch(release, "release", 30);
                    });
                    return null;
                ...
                Future<Object> b = pool.submit(() -> {
                    lockProbe.armCurrentThread(LockPoint.ATTEMPT_FOR_UPDATE);
                    return wrap(() -> submitService.submitAttempt(f.studentId, f.attemptId,
                            new SubmitRequest("kb-" + UUID.randomUUID())));
                });
                assertThat(lockProbe.awaitEntered(LockPoint.ATTEMPT_FOR_UPDATE, 10))
                        .as("worker B reached AttemptRepository.findByIdForUpdate").isTrue();
                assertThat(b.isDone()).as("worker B must be blocked on A's attempt lock (true overlap)").isFalse();
    ...
    void submitVsAutosaveProducesOneSubmitEventPair() throws Exception {
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            ExecutorService pool = Executors.newFixedThreadPool(2);
            try {
                // Worker A: outer tx → autosave (acquires attempt lock) → signal held → await
                // release.
                Future<?> a = pool.submit(() -> tx().executeWithoutResult(status -> {
                ...
                assertThat(awaitLatch(aHeld, "aHeld", 15)).as("worker A (autosave) entered tx + locked attempt")
                        .isTrue();
                Future<Object> b = pool.submit(() -> {
                    lockProbe.armCurrentThread(LockPoint.ATTEMPT_FOR_UPDATE);
                    return wrap(() -> submitService.submitAttempt(f.studentId, f.attemptId,
                            new SubmitRequest("svs-" + UUID.randomUUID())));
                });
                assertThat(lockProbe.awaitEntered(LockPoint.ATTEMPT_FOR_UPDATE, 10))
                        .as("worker B reached AttemptRepository.findByIdForUpdate").isTrue();
                assertThat(b.isDone()).as("worker B (submit) must be blocked on A's attempt lock (true overlap)")
                        .isFalse();
                release.countDown();
                a.get(30, TimeUnit.SECONDS);
                b.get(30, TimeUnit.SECONDS);
    ...
    // === §3.5 two-student concurrent start (FRESH, zero attempts) + true-overlap
    // barrier ===
    @Test
    void twoStudentConcurrentStartProducesTwoEventPairs() throws Exception {
        ...
        try (StompTestConnection teacher = connect(f.teacherToken)) {
            ...
            ExecutorService pool = Executors.newFixedThreadPool(2);
            try {
                // Worker A (s1): outer tx → startAttempt (acquires the SESSION lock) → signal →
                // await release.
                Future<?> a = pool.submit(() -> tx().executeWithoutResult(status -> {
                ...
                assertThat(lockProbe.awaitEntered(LockPoint.SESSION_FOR_UPDATE, 10))
                        .as("worker B reached ExamSessionRepository.findByIdForUpdate").isTrue();
            ...
            // Each attemptId appears exactly once across the ATTEMPT_STARTED events (no
            // duplicate).
            Set<Long> attemptIds = new HashSet<>();
            for (RealtimeEventEnvelope ev : parse(cap.payloads)) {
                if (ev.eventType() != null && ev.eventType().contains("ATTEMPT_STARTED"))
    ...
    private ExistingAttempt existingAttempt() {
        FreshSession f = freshSession();
        long attemptId = attemptService.startAttempt(f.studentId, f.sessionId, new StartAttemptRequest(null))
                .attemptId();
        long aq = jdbc.queryForObject("SELECT id FROM attempt_questions WHERE attempt_id=" + attemptId + " LIMIT 1",
                Long.class);
    ...
    private long addStudent(String tag, long school, long session, long teacher) {
        ...
        ins("INSERT INTO student_profiles (user_id, school_id, student_code) VALUES (" + uid + "," + school + ",'SC"
                + tag + "')");
        jdbc.update(
                "INSERT INTO exam_session_participants (school_id, exam_session_id, student_profile_id, added_by) VALUES ("
                        + school + "," + session + ",(SELECT id FROM student_profiles WHERE user_id=" + uid + "),"
                        + teacher + ")");
        return uid;
    }
    ...
    private static String topic(long sid) {
        return "/topic/exam-sessions/" + sid;
    }
    ...
    /**
     * DELIVERY_ORDER_UNSPECIFIED — assert by exact count of each type, not by
     * index.
     */
    private void assertEventCounts(List<byte[]> payloads, int attemptEvents, int countEvents) {
        ...
        long distinctIds = java.util.Arrays.stream(ev).map(RealtimeEventEnvelope::eventId)
                .filter(java.util.Objects::nonNull).distinct().count();
        assertThat(distinctIds).as("every event carries a distinct eventId (no duplicate)").isEqualTo(ev.length);
    }

    /**
     * After the expected latch count is received, asserts no
     * extra/delayed-duplicate frame in a bounded window.
     */
    ...
    private long activeCount(long sessionId) {
        return jdbc.queryForObject("SELECT count(*) FROM attempts WHERE exam_session_id=? AND status='IN_PROGRESS'",
                Long.class, sessionId);
    }

    private long idempotencyRows(long attemptId) {
        return jdbc.queryForObject("SELECT count(*) FROM idempotency_records WHERE attempt_id=?", Long.class,
                attemptId);
    }
    ...
    private static Object wrap(Body body) {
        try {
            return body.run();
