...
/**
 * Day 8 teacher/admin session results — one BEST row per student, paginated, with authorization.
 * Uses PostgreSQL CTE + {@code ROW_NUMBER()} window function for BEST selection (matches
 * {@link BestResultComparator}). {@code COUNT(*) OVER} provides per-student attempt count with no N+1.
 * Sort/filter use hardcoded SQL fragments with parameter binding; database {@code LIMIT}/{@code OFFSET}.
 */
@Service
@Transactional(readOnly = true)
public class SessionResultService {

    private static final Set<String> SORT_ALLOWLIST = Set.of("percentage", "score", "submittedAt", "studentName", "studentCode");
    ...
    /**
     * Returns a paginated list of BEST results for a session using PostgreSQL CTE + window function.
     * Authorization: teacher must own the session; SYSTEM_ADMIN/ACADEMIC_ADMIN allowed; STUDENT denied.
     * No N+1 — attempt_count via COUNT(*) OVER; no per-student count query; database LIMIT/OFFSET.
     */
    public SessionResultsPage getSessionResults(Long userId, String primaryRole, Long sessionId,
        ...
        if (page < 0) throw new AttemptException(AttemptErrorCode.INVALID_RESULT_QUERY);
        if (size <= 0 || size > MAX_PAGE_SIZE) throw new AttemptException(AttemptErrorCode.INVALID_RESULT_QUERY);
        if (direction != null && !direction.isBlank() && !"ASC".equalsIgnoreCase(direction) && !"DESC".equalsIgnoreCase(direction))
        ...
        if (!SORT_ALLOWLIST.contains(sortField)) throw new AttemptException(AttemptErrorCode.INVALID_RESULT_QUERY);
        ...
        // Build filter conditions (hardcoded SQL fragments; user values via parameter binding)
        ...
        if (minPct != null) { filterSql.append(" AND percentage >= ?"); filterArgs.add(minPct); }
        if (maxPct != null) { filterSql.append(" AND percentage <= ?"); filterArgs.add(maxPct); }
        ...
        String pageSql = "SELECT * FROM (" + baseCte + ") ranked WHERE rn = 1" + filters
                + " ORDER BY " + sortColumn + " " + sortDir + " NULLS LAST, submitted_at ASC NULLS LAST, student_id ASC LIMIT ? OFFSET ?";
    ...
    /** Returns ALL BEST rows for the session (no pagination/filter) — for statistics/export reuse. */
    ...
    /** Package-private authorization check — fail-closed: unsupported/null roles are denied. */
    void authorizeSessionAccess(Long userId, String primaryRole, Long sessionId) {
        ...
        switch (primaryRole) {
            ...
            case "SYSTEM_ADMIN", "ACADEMIC_ADMIN" -> { /* allowed */ }
            case "TEACHER" -> {
    ...
    /** Package-private raw BEST query — shared by results, statistics, and export. No N+1 (CTE + window). */
