...
/**
 * Day 8 result read service — reads persisted Grade/GradeItems, builds student-facing DTOs with the BEST
 * flag, and enforces authorization. NEVER re-grades, NEVER reads the answer key. The {@code answered} flag
 * is derived from persisted {@link AttemptAnswer#getAnswerPayload()} (non-null = answered).
 */
@Service
...
public class AttemptResultService {
    ...
    /** Role-aware attempt result (reads persisted snapshot; no re-grading; no answer key). */
    ...
    /** Student own BEST result for a session (RESULT_NOT_FOUND if no submitted+graded attempt). */
    ...
    private AttemptResultResponse buildResponse(Attempt attempt, Grade grade, Long studentProfileId) {
        ...
        List<AttemptQuestion> questions = attemptQuestionRepo.findByAttemptIdOrderByDisplayOrderAsc(attempt.getId());
        ...
        // isBest + attemptCount — batch load grades for all submitted attempts of this (session, student).
        ...
        List<BestCandidate> candidates = submittedIds.stream()
                .map(id -> {
                    ...
                    return new BestCandidate(a.getId(), g.getPercentage(), g.getFinalScore(), a.getSubmittedAt());
                })
        ...
        List<QuestionResultView> questionResults = questions.stream()
                .map(q -> {
                    ...
                    return new QuestionResultView(q.getId(), q.getExamQuestionId(), q.getQuestionType(),
    ...
    /** Finds the BEST attempt id for (session, student) using the frozen comparator, or null if none. */
    private Long findBestAttemptId(Long sessionId, Long studentProfileId) {
        ...
        List<Long> submittedIds = all.stream()
                .filter(a -> a.getStatus() == AttemptStatus.SUBMITTED || a.getStatus() == AttemptStatus.GRADED)
                .map(Attempt::getId).toList();
        if (submittedIds.isEmpty()) return null;
        Map<Long, Grade> gradeByAttempt = gradeRepo.findAllByAttemptIdIn(submittedIds).stream()
                .collect(Collectors.toMap(Grade::getAttemptId, g -> g, (a, b) -> a));
